  ...
  //exit;
   ...
   //Insert basic details
               if (!empty($data['10th_spcl'])) {
                   ...
                   $query = 'INSERT INTO `xth_details` (`user_id`, `school`, `board`, `percentage`, `marks`, `passing_year`) VALUES (' . $data['user_id'] . ',"' . $data['10th_school'] . '","' . $data['10th_university'] . '","' . $data['10th_percentage'] . '","' . $data['10th_marks'] . '","' . $data['10th_Pyear'] . '")';
                   $select = $this->pdo->prepare($query);
                   $select->execute();
       ...
       function updateEducationDetails($data) {
           if (!empty($data['10th_spcl'])) {
               $query = 'UPDATE `xth_details` SET `school`="' . $data['10th_school'] . '",`board`="' . $data['10th_university'] . '",`percentage`="' . $data['10th_percentage'] . '",`marks`="' . $data['10th_marks'] . '",`passing_year`="' . $data['10th_Pyear'] . '" WHERE `user_id`=' . $data['user_id'] . '';
               $select = $this->pdo->prepare($query);
               $select->execute();
               $data['qualification_id'] = $this->getQualificationidbyNames('10th');
           }
...
//print_r($candidate->query($data));
   ...
   //print_r($sel);exit;
     ...
     function getJobsSearch_pagination_pages($no_of_records_per_page,$data) {		$location=$data['location'];$keyword=$data['keyword'];		             

           //$query = "select count(*) from jobs where active='1' ORDER BY posted_on DESC";
           if(empty($data)){  
							 $query = "select count(*) from jobs where active='1' ORDER BY posted_on DESC"; 
		   }elseif(!empty($data['location'])){
			              $query = "select count(*) from jobs where active='1' AND location LIKE '%" . $location . "%'  ORDER BY 	posted_on DESC";

		   }elseif(!empty($data['keyword'])){
			$query = "select count(*) from jobs where active='1' AND location LIKE '%" . $keyword . "%' OR designation LIKE '%" . $keyword . "%'  OR  description LIKE '%" . $keyword . "%' ORDER BY posted_on DESC";
       ...
       function getJobsSearch_pagination_records($no_of_records_per_page, $offset,$data) {		$location=$data['location'];$keyword=$data['keyword'];		             
           ...
           if(empty($data)){  
			               ...
			               $query = "select * from jobs  where active='1' AND location LIKE '%" . $location . "%'  ORDER BY posted_on DESC limit $offset,$no_of_records_per_page ";

		   }elseif(!empty($data['keyword'])){
			  $query = "select * from jobs  where active='1' AND location LIKE '%" . $keyword . "%' OR ug_qualification LIKE '%" . $keyword . "%' OR pg_qualification LIKE '%" . $keyword . "%' OR designation LIKE '%" . $keyword . "%'  OR  description LIKE '%" . $keyword . "%'  OR  job_name LIKE '%" . $keyword . "%'  OR  company_name LIKE '%" . $keyword . "%'  ORDER BY posted_on DESC limit $offset,$no_of_records_per_page ";
    ...
    function getJobsSearch($data) {	//print_r($data);
		$location=$data['location'];$keyword=$data['keyword'];		             
		   if(empty($data)){  
							$select = "SELECT DISTINCT * FROM jobs ORDER BY 	posted_on DESC"; 
		   }elseif(!empty($data['location'])){
			              $select = "SELECT  * FROM jobs WHERE  location LIKE '%" . $location . "%'  ORDER BY 	posted_on DESC";

		   }elseif(!empty($data['keyword'])){
			              echo $select = "SELECT  * FROM jobs WHERE location LIKE '%" . $keyword . "%' OR designation LIKE '%" . $keyword . "%'  OR  description LIKE '%" . $keyword . "%' ORDER BY posted_on DESC";
...
//echo $row['type'];
   ...
   //print_r($data);exit;
       ...
       function addPersonal($data) {
           //print_r($data);
           try {
               $update = $this->pdo->prepare("UPDATE `user_profile` SET `date_of_birth`=?,`permanent_address`=?,`current_address`=?,`passport_number`=? WHERE user_id = ?");
   
               $update->execute(array($data['date_of_birth'], $data['permanent_address'], $data['current_address'], $data['passport_number'], $data['get_id']));
           } catch (PDOException $e) {
               $this->setError($e->getMessage());
               return false;
           }
   ...
   <tr><th  width="40%"  style="border: none;">Domain</th><td style="border: none;">' . $education[0]['domain'] . '</td></tr><tr><th  width="40%"  style="border: none;">All India Rank</th><td style="border: none;">' . $education[0]['all_india_score'] . '</td></tr><!--tr> <th  width="40%"  style="border: none;">Marks</th><td style="border: none;">' . $education[0]['marks'] . '</td></tr--><tr> <th  width="40%"  style="border: none;">Year Of Passing</th><td style="border: none;">' . $education[0]['year_of_passing'] . '</td></tr> </table></div>';
     ...
     echo $education;
       ...
       function jobsearch_pagination_pages($no_of_records_per_page, $data) {
           try {
					...
					if(empty($data)){  
						...
						$select = "select count(*) from jobs where active='1' AND location LIKE '%" . $location . "%'  ORDER BY 	posted_on DESC";

					}elseif(!empty($data['role'])){
						 $select = "select count(*) from jobs where active='1' AND designation LIKE '%" . $data['role'] . "%'   ORDER BY posted_on DESC";

					}elseif(!empty($data['job_name'])){
						 $select = "select count(*) from jobs where active='1' AND job_name LIKE '%" .$data['job_name']. "%'    ORDER BY posted_on DESC";

					}
					elseif(!empty($data['job_name']) && !empty($data['role']) && !empty($data['location'])){
						$query = "select count(*) from jobs where active='1' AND   job_name LIKE '%" .$data['job_name']. "%'  OR 
						location LIKE '%" .$data['location'] . "%' OR 
						designation LIKE '%" . $data['role'] . "%'   ORDER BY posted_on DESC ";
					}
               $select = $this->pdo->prepare($query);
               $select->execute();
               $num_of_rows = $select->fetchColumn();
      ...
      ///  echo   $query = "select * from jobs where active='1' AND   job_name LIKE '%" .$data['job_name']. "%'  OR location LIKE '%" .$data['location'] . "%' OR designation LIKE '%" . $data['role'] . "%'   ORDER BY posted_on DESC   limit $offset,$no_of_records_per_page";
                   
                   if(empty($data)){  
						$query = "select * from jobs where active='1' ORDER BY 	posted_on DESC limit $offset,$no_of_records_per_page ";
					}elseif(!empty($data['location'])){
						$query = "select * from jobs where active='1' AND location LIKE '%" . $data['location'] . "%'  ORDER BY 	posted_on DESC limit $offset,$no_of_records_per_page";

					}elseif(!empty($data['role'])){
						 $query = "select * from jobs where active='1' AND designation LIKE '%" . $data['role'] . "%'   ORDER BY posted_on DESC limit $offset,$no_of_records_per_page";

					}elseif(!empty($data['job_name'])){
						 $query = "select * from jobs where active='1' AND job_name LIKE '%" .$data['job_name']. "%'    ORDER BY posted_on DESC limit $offset,$no_of_records_per_page";

					}
					elseif(!empty($data['job_name']) && !empty($data['role']) && !empty($data['location'])){
						$query = "select * from jobs where active='1' AND   job_name LIKE '%" .$data['job_name']. "%'  OR  location LIKE '%" .$data['location'] . "%' OR   designation LIKE '%" . $data['role'] . "%'   ORDER BY posted_on DESC limit $offset,$no_of_records_per_page";
					}
                // echo $query;  
           $select = $this->pdo->prepare($query);
           $select->execute();
